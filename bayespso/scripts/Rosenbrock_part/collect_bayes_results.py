"""Call with 'python'
This is not for the final plotting, but for the one done immediately after finishing
calculations

Usage: collect_bayes_results.py.py --input_dir=DIR --output_dir=DIR
...
def main(input_dir, output_dir):
    base_dir = os.path.join(input_dir, 'BayesianOpt')
    ...
    for chunk_size in CHUNK_SIZES:
        chunk_name = 'batch_%s' %chunk_size
...
def create_distrib_plots(iter_dicts, collected_repeats_dir):
    ...
    plt.title(
        'PSO best found location distributions with different batch sizes')
    plt.grid()
    plt.savefig(output_path, bbox_inches='tight')
    plt.close('all')
...
def create_eval_wise_plot(iter_dicts, collected_repeats_dir):
    ...
    plt.xlim(0, 10000)
    plt.yscale('log')
    plt.title('Bayesian evolution with different batch sizes')
    plt.savefig(output_path, bbox_inches='tight')
    plt.close('all')
        ...
        for path in glob.glob(wild_card_path):
            with open(path, 'rt') as info_file:
                for line in info_file:
                    d = json.loads(line)
                    iter_dict[d['iteration']].append(d['best_fitness'])
    return iter_dict
